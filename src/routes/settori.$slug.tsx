...
export const Route = createFileRoute("/settori/$slug")({
  loader: async ({ params }): Promise<{ sector: SectorView }> => {
    // Prima il settore gestito da pannello admin; se il database non risponde
    // o non ha quello slug si usa l'elenco statico.
    let row: unknown = null;
    try {
      row = await getSectorBySlug({ data: { slug: params.slug } });
    } catch {
      row = null;
    }
    if (row) {
      return { sector: sectorFromRow(row as Parameters<typeof sectorFromRow>[0]) };
    }
    const fallback = staticSectors.find((s) => s.slug === params.slug);
    if (!fallback) throw notFound();
    return { sector: sectorFromStatic(fallback) };
  },
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
  ...
  notFoundComponent: () => (
    <div className="container-px mx-auto max-w-7xl pt-40 pb-20">
      ...
      <Link
        to="/settori"
        className="mt-6 inline-flex items-center gap-2 text-electric font-semibold"
      >
    ...
    <>
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 bg-ink text-ink-foreground overflow-hidden">
        ...
        <div className="relative container-px mx-auto max-w-7xl">
          <Link
            to="/settori"
            className="inline-flex items-center gap-2 text-sm text-white/70 hover:text-electric transition-colors mb-8"
          >
        ...
        <div className="grid lg:grid-cols-12 gap-16">
          ...
          <aside className="lg:col-span-5 space-y-6">
            ...
            <div className="rounded-2xl bg-ink text-ink-foreground p-8">
              <div className="text-xs uppercase tracking-[0.22em] text-electric mb-3">
                Hai un progetto?
              </div>
              <h3 className="font-display text-2xl font-bold leading-snug">
                Parla con un nostro tecnico in 24h.
              </h3>
              <Link
                to="/contatti"
                className="mt-6 inline-flex items-center gap-2 rounded-full bg-electric text-electric-foreground px-6 py-3 font-semibold"
              >
