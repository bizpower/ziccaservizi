      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            ISO, SOA e altre certificazioni dell'azienda.
          </p>
        </div>
        <button
        ...
        <div className="grid gap-3">
          {(q.data ?? []).map((c: any) => (
            ...
            >
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full inline-flex items-center gap-1 ${c.published ? "bg-electric/10 text-electric" : "bg-muted text-muted-foreground"}`}
              >
                {c.published ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}
                {c.published ? "Online" : "Bozza"}
              </span>
              <button onClick={() => setEditing(c)} className="p-2 rounded-md hover:bg-muted">
          ...
          <div className="bg-background rounded-2xl border border-border max-w-2xl w-full p-6 my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-display text-xl font-bold">
                {editing.id ? "Modifica certificazione" : "Nuova certificazione"}
              </h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <MediaUpload
                ...
                onChange={(url) => setEditing({ ...editing, pdf_url: url })}
                folder="certifications/pdf"
                accept="application/pdf"
                label="PDF certificato (opzionale)"
              />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Ordine">
                  <input
